...
<head>
	...
	button.addDelivery
		...
		font-size: 2vw;	
		text-align: center;
		background: rgba(222, 241, 250, 1.0);
		border-radius: 8px;
		padding: 4px 20px;
	...
	button.submitID
		...
		font-size: 1.0vw;	
		text-align: center;
		background: rgba(111, 231, 251, 0.57);
		padding: 1px 10px;
		box-sizing: border-box;
		width: 7vw;
		margin-top: 0.3vw;
...
<?php
	...
	//Shows form for adding parts from delivery to inventory
		...
		$sql = "SELECT " . $dbPartID . ", " . $dbPartDescription . " FROM " . $dbCatalog . " ORDER BY " . $dbPartID . " ASC;";
		$result = $pdo->query($sql);
		$combinedArrayID = $result->fetchAll();

		//Creates combined array sorted by Description
		$sql = "SELECT " . $dbPartID . ", " . $dbPartDescription . " FROM " . $dbCatalog . 
		" ORDER BY " . $dbPartDescription . " ASC;";
		$result = $pdo->query($sql);
		$combinedArrayDesc = $result->fetchAll();

		//Creates array of IDs
		$sql = "SELECT " . $dbPartID . " FROM " . $dbCatalog . 
		" ORDER BY " . $dbPartID . " ASC;";
		$result = $pdo->query($sql);
		$arrayID = $result->fetchAll();
		$searchArrayID = array_column($arrayID, $dbPartID);
		
		//Creates array of descriptions
		$sql = "SELECT " . $dbPartDescription . " FROM " . $dbCatalog . 
		" ORDER BY " . $dbPartDescription . " ASC;";
		$result = $pdo->query($sql);
